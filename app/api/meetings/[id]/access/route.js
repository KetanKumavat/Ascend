...
import { getCachedMeetings } from "@/lib/cache";
...
export async function GET(request, { params }) {
  try {
    ...
    const canViewMeetCode = meeting.meetingUrl && (
    ...
    if (meeting.meetingUrl) {
      const googleMeetMatch = meeting.meetingUrl.match(/meet\.google\.com\/([a-z-]+)/);
      if (googleMeetMatch) {
    ...
    return NextResponse.json({
      ...
      meetCode,
      canJoin: true,
      isParticipant: meeting.participants.some(p => p.userId === userId),
