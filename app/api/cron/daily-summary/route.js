...
export async function POST(request) {
    try {
        ...
        // Check for authorization (you might want to use an API key for cron jobs)
        ...
        const projectsWithCommits = await prisma.project.findMany({
            where: {
                commits: {
                    some: {
                        commitDate: {
        ...
        for (const project of projectsWithCommits) {
            try {
                ...
                if (!existingSummary) {
                    ...
                    const summaryResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/daily-summary`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (summaryResponse.ok) {
                        ...
                        if (summaryData.success && summaryData.summary) {
                            ...
                            const emailResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/send-daily-summary`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Authorization': `Bearer ${process.env.CRON_SECRET}`
